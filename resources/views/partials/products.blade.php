...
$services_bk = [
    ...
    [
        ...
        'style' => 'width:55px', // special inline style
    ],
    [
        'filter_class' => 'filter-tour',
        'title' => 'Air Tickets',
...
<section id="portfolio" class="portfolio">
    ...
    <div class="container" data-aos="fade-up">
        ...
        <div class="row portfolio-container" data-aos="fade-up" data-aos-delay="200">
            ...
            @php
                // Assuming $services array is defined and available in your Blade view
                $theme_path = 'public/theme/web-new/'; // Base path for local images
            @endphp

            @foreach ($services as $service)
                {{-- The filter-class key maps to the category filter class --}}
                <div class="col-lg-3 col-md-6 portfolio-item {{ $service['filter_class'] }}">
                    <div class="innerBox">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="portfolio-img">
                                    {{-- Check if image is external (starts with http) or local --}}
                                    @php
                                        $img_src =
                                            strpos($service['image'], 'http') === 0
                                                ? $service['image']
                                                : asset($theme_path . $service['image']);
                                    @endphp
                                    <img src="{{ $img_src }}" class="img-fluid" alt="{{ $service['alt'] }}"
                                        @if (isset($service['style'])) style="{{ $service['style'] }}" @endif />
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="portfolio-info">
                                ...
                                <a href="{{ route('enquiry.create',$service['id']) }}" class="details-link" title="More Details">More</a>
                            </div>
                        </div>
                    </div>
                </div>
